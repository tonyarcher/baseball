...
* **Database Entities**: JPA `@Entity` classes isolating database table schemas. Maps database tables to/from shared
  domain models.
* **Exception Handling**: Catch domain exceptions via a global `@RestControllerAdvice` and map them to appropriate HTTP
  status codes.

## 3. Testing Protocols

* **Slice Tests**: Use JUnit 5 and Spring slice tests (`@WebMvcTest`, `@DataJpaTest`) to test HTTP contracts and
