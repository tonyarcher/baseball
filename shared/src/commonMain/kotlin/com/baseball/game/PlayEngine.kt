...
object PlayEngine {
    fun processPlay(
        ...
        val batter = (state.awayRoster + state.homeRoster).find { it.id == batterId } 
            ?: throw IllegalArgumentException("Batter not found in rosters")
        val pitcher = (state.awayRoster + state.homeRoster).find { it.id == pitcherId } 
        ...
        var bStats = (if (batter.teamId == game.homeTeam.id) homeBattingList else awayBattingList)
            .find { it.playerId == batterId } ?: PlayerBattingStats(batterId, batter.name, batter.jerseyNumber, batter.position)
        var pStats = (if (pitcher.teamId == game.homeTeam.id) homePitchingList else awayPitchingList)
        ...
        runsScoredList.forEach { runnerId ->
            ...
            var runBStats = battingList.find { it.playerId == batterId } ?: PlayerBattingStats(batterId, batter.name, batter.jerseyNumber, batter.position)
            ...
            if (runnerId != batterId) {
                ...
                val runnerBattingList = if (runner.teamId == game.homeTeam.id) boxScore.homeBatting.toMutableList() else boxScore.awayBatting.toMutableList()
            ...
            var runPStats = pitchingList.find { it.playerId == pitcherId } ?: PlayerPitchingStats(pitcherId, pitcher.name, pitcher.jerseyNumber, pitcher.position)
        ...
        val updatedSessionState = state.copy(
            game = updatedGame,
            boxScore = boxScore,
