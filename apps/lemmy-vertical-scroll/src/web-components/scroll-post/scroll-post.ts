...
export class ScrollPost extends LitElement {
    ...
    /** Videos collapse the metadata to a bar so native controls stay clear. */
    @state() private expanded = false

    override willUpdate(changed: Map<string, unknown>): void {
        if (changed.has('post')) {
            this.expanded = classifyPost(this.post ?? ({name: ''} as LemmyPost)) !== 'video'
        }
    }
    ...
    override render(): TemplateResult {
        ...
        return html`
            <div class="scroll-post">
                ...
                ${this.renderLinkChip()}
                <div class="post-overlay${isVideo ? ' video' : ''}${this.expanded ? ' expanded' : ''}">
                    ...
                    ${this.expanded ? html`<h3 class="post-title">${post.name}</h3>` : nothing}
                </div>
            </div>
        `
    }
