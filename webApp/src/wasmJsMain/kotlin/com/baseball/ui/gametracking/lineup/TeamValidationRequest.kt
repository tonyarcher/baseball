...
internal fun getTeamValidationError(request: TeamValidationRequest): String? {
    ...
    val nums = request.list.map { it.jerseyNumber.toIntOrNull() }
    val allNums = if (request.useDh) nums + request.pNum.toIntOrNull() else nums
...
internal fun checkEmptyNames(request: TeamValidationRequest, teamName: String): String? =
    if (request.list.any { it.name.trim().isEmpty() })
...
internal fun checkPitcherWhenDh(request: TeamValidationRequest, teamName: String): String? =
    if (request.useDh && (request.pName.trim().isEmpty() || request.pNum.toIntOrNull() == null))
...
internal fun checkPitcherRequirements(request: TeamValidationRequest, teamName: String): String? {
    val pCount = request.list.count { it.position == "P" }
    return when {
        !request.useDh && request.list.indexOfFirst { it.position == "P" } == -1 ->
